...
def GUI2Function():
    ...
    tree = ttk.Treeview(TableMargin, columns=("Name", "Class", "State", "Price"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    ...
    with open('metadata.csv', encoding='UTF8') as f:
        ...
        for row in reader:
            ...
            tree.insert("", 0, values=(name, classi, state, price))
...
def mainFunction():
    ...
    if nameOfItem.startswith("Apple Mac"):
        data = [nameOfItem[6:], classOfItem[-1], stateOfItem, priceOfItem]
    else:
        data = [nameOfItem, classOfItem[-1], stateOfItem, priceOfItem]
